  ...
  return (
    <div className="hidden md:flex flex-col w-64 bg-black p-4 gap-y-6 overflow-auto border-r border-zinc-800/30 rounded-lg">
      ...
      <div className="space-y-1">
        {renderNavItem('/', <HomeIcon size={20} />, 'Home')}
        {renderNavItem('/search', <Search size={20} />, 'Search')}
        {renderNavItem('/library', <ListMusic size={20} />, 'Your Library')}
      </div>
      ...
      <div className="space-y-6 mt-6">
        <div className="space-y-1">
          <button
            className="flex items-center gap-3 text-zinc-400 font-medium hover:text-white transition-colors py-2 w-full text-left"
        ...
        <div className="space-y-2 pl-2">
          <h3 className="text-xs font-bold text-zinc-500 mb-2 tracking-wider">YOUR PLAYLISTS</h3>
          ...
          {isAuthenticated ? (
            userPlaylists.length > 0 ? (
              ...
              <p className="text-zinc-500 text-sm font-medium">No playlists yet</p>
            )
          ) : (
            <p className="text-zinc-500 text-sm font-medium">Sign in to see playlists</p>
          )}

          {/* Sample playlists for UI - remove these when user playlists are available */}
          {!isAuthenticated && (
            <>
              <p className="text-zinc-400 py-1.5 hover:text-white transition-colors text-sm font-medium cursor-pointer">
                Discover Weekly
              </p>
              <p className="text-zinc-400 py-1.5 hover:text-white transition-colors text-sm font-medium cursor-pointer">
                Release Radar
      ...
      <div className="mt-auto pt-6">
        <Button
          variant="ghost"
          className={cn(
            'flex items-center gap-2 text-zinc-400 hover:text-white transition-colors w-full justify-start font-medium p-2'
          )}
        >
          <Download size={16} />
