...
export const getSyncedLikedSongs = async (userId: string) => {
  try {
    const response = await axios.get(`/spotify/liked-songs/${userId}`);
...
export const getSyncStatus = async (userId: string) => {
  try {
    const response = await axios.get(`/spotify/sync-status/${userId}`);
    return response.data;
  } catch (error) {
...
export const triggerManualSync = async (userId: string) => {
  try {
    const response = await axios.post('/spotify/sync', { userId });
    return response.data;
  } catch (error) {
...
export const handleSpotifyLikeUnlike = async (userId: string, trackId: string, action: 'like' | 'unlike') => {
  try {
    const response = await axios.post('/spotify/like-unlike', {
      userId,
      trackId,
      action
    });
...
export const forceRefreshSyncedSongs = async (userId: string) => {
  try {
    ...
    const response = await axios.get(`/spotify/liked-songs/${userId}?refresh=true`);
    return response.data;
  } catch (error) {
...
export const deleteAllLikedSongs = async (userId: string) => {
  try {
    const response = await axios.delete(`/spotify/liked-songs/${userId}`);
    return response.data;
  } catch (error) {
...
export const migrateLikedSongsStructure = async (userId: string) => {
  try {
    const response = await axios.post(`/spotify/migrate/${userId}`);
    return response.data;
  } catch (error) {
